...
for meeting in name_of_meetings:
	...
	if speakers_words == None:
		continue

	# getting default topics
	default_topics_dict = parse_default_topics(data_root, ontologies_folder)

	words_in_topics, topic_description = get_words_in_topics(data_root, topics_folder, meeting, default_topics_dict, speakers_words)
	...
	for c in string.ascii_uppercase:
		try:
			speaker_das, speaker_das_descriptions =  get_speaker_dialogue_acts(data_root, dialogue_acts_folder, 
				meeting, speakers_words, default_das_dict, c)
		...
		save_meeting_dialogue_acts(output_root, meeting, words_in_topics, c)
		save_meeting_dialogue_acts_descriptions(output_root, meeting, words_in_topics, c)

#     # extractive_summary
#     try:
#         summ = ET.parse(os.path.join(data_root, extractive_sum_folder, meeting + '.extsumm.xml'))
#         root = summ.getroot()
#         das_in_summary = []

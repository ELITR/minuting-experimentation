...
for rec in rec_list:
	try:
		# split in 3 parts
		temp_list = rec.split('descriptions: ')
		a = temp_list[0]
		b = temp_list[1]
		temp_list = b.split('abst_sum - ')
		c = temp_list[0]
		d = temp_list[1]
	except:
		continue
	...
	da = clean_text(c, keep_speakers=False, lower=True)
	abst = clean_text(d, keep_speakers=False, lower=True)
	src_list.append(src)
	da_list.append(da)
...
write_src = "{}/{}".format(args.dest, "src.txt")
write_da = "{}/{}".format(args.dest, "da.txt")
write_abst = "{}/{}".format(args.dest, "abst.txt")
